...
const Watermeter: FC = () => {
  ...
  const fetchWatermeter = async () => {
    try {
      const response = await axiosConfig.get("/api/data_water");
  ...
  return (
    <Layout>
      {popupImage && (
        <div className="fixed top-0 left-0 w-full h-full flex bg-black bg-opacity-50 justify-center items-center duration-300 z-50 ">
          <div className="max-w-[90%] max-h-[90%]">
            <img src={popupImage} alt="Popup" className="max-w-full max-h-full rounded-md" />
            <button className="absolute top-0 right-0 m-4 p-2 rounded-full text-white" onClick={handleClosePopup}>
              X
            </button>
          </div>
        </div>
      ...
      <div className="w-full py-6 px-8 justify-start overflow-y-auto h-screen pb-24">
        <h1 className="text-black/80 text-[32px] font-[600]">มิเตอร์น้ำ</h1>
